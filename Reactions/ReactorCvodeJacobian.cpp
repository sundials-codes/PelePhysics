  ...
  N_Vector /*tmp3*/)
{
  ...
  if (solveType == sparseDirect) {
    ...
    if (auto J = dynamic_cast<sundials::ginkgo::BlockMatrix<gko::matrix::BatchCsr<amrex::Real>>*>) {
      auto Jgko = J->GkoMtx();
      ...
      amrex::Gpu::Device::streamSynchronize();
    } else if (auto J = sundials::ginkgo::BlockMatrix<gko::matrix::BatchDense<amrex::Real>>*) {
      amrex::Abort("ginkgo::matrix::BatchDense not supported!");
      // auto Jgko = J->GkoMtx();
      // amrex::Real* yvec_d  = N_VGetDeviceArrayPointer(y_in);
      // amrex::Real* Jdata   = Jgko->get_values();
